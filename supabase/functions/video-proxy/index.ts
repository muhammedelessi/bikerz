...
const generateBunnyToken = async (
  ...
  // Convert hex to base64 and make URL-safe
...
serve(async (req) => {
  ...
  try {
    ...
    if (!BUNNY_CDN_PATTERN.test(decodedUrl)) {
      return new Response(
        JSON.stringify({ error: 'Only Bunny CDN URLs are allowed' }),
    ...
    console.log(`[video-proxy] Fetching: ${fetchUrl.substring(0, 100)}...`);
...
async function handleSuccessResponse(response: Response, originalUrl: string, requestUrl: URL): Promise<Response> {
