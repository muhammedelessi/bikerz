...
-- Update grade_lesson_activity to handle 'informational' question type
CREATE OR REPLACE FUNCTION public.grade_lesson_activity(p_activity_id UUID, p_user_answers TEXT[])
...
BEGIN
  ...
  v_question_type := v_activity.data->>'question_type';

  -- Informational questions: always correct if any answer provided
  IF v_question_type = 'informational' THEN
    v_is_correct := (array_length(p_user_answers, 1) > 0);
  ELSE
