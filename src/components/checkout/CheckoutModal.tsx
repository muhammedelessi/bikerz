...
const CheckoutModal: React.FC<CheckoutModalProps> = memo(
  ({ open, onOpenChange, course, onSuccess, onPaymentStarted }) => {
      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="sm:max-w-lg max-h-[90vh] flex flex-col overflow-hidden border-2 border-border bg-card">
          ...
          <div className="flex-shrink-0 border-t border-border pt-3 flex gap-2">
            ...
            <Button
              className="flex-1"
              disabled={step === "info" ? !form.isInfoValid : !isPaymentReady || guestSigningUp}
              ...
              {step === "info"
                ...
                : discountedPrice === 0
                  ? isRTL
                    ...
                    : "Enroll Free"
                  : isRTL
                    ? `ادفع ${formatLocal(discountedPrice)}`
                    : `Pay ${formatLocal(discountedPrice)}`}
            </Button>
          </div>
        </DialogContent>
      </Dialog>
