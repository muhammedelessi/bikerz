                ...
                return (
                  <div key={entry.id} className="rounded-2xl border border-border/50 overflow-hidden bg-card shadow-sm hover:shadow-md transition-shadow flex flex-col">
                    {entry.photos.length > 0 ? (
                      <div className="relative aspect-[6/4] overflow-hidden rounded-t-2xl bg-muted shrink-0">
                        ...
                        {entry.photos.length > 1 && (
                          <div className="absolute bottom-2 end-2 bg-black/60 text-white text-[10px] font-semibold px-2 py-0.5 rounded-full backdrop-blur-sm">1 / {entry.photos.length}</div>
                        )}
                      </div>
                    ) : (
                      <div className={cn('aspect-[6/4] rounded-t-2xl bg-gradient-to-b flex items-center justify-center shrink-0', gradient)}>
                    ...
                    <div className="p-3 space-y-1.5 flex-1">
                      ...
                      <div>
                        {entry.brand && <p className="text-[10px] text-muted-foreground font-medium tracking-wide uppercase" dir="ltr">{entry.brand}</p>}
                        <p className="text-sm font-black text-foreground leading-tight" dir="ltr">{entry.model || (isRTL ? 'دراجة غير معرّفة' : 'Unknown Bike')}</p>
                      </div>
                    </div>
