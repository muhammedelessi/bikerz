...
import { Star, Users, MapPin, Bike, Clock, BookOpen, User, ChevronDown, Plus } from "lucide-react";
...
const TrainingSection: React.FC<{
  ...
  const avgRating = trainingReviews.length
    ? (trainingReviews.reduce((a: number, r: any) => a + r.rating, 0) / trainingReviews.length).toFixed(1)
    : null;
...
const TrainerProfileDialog: React.FC<TrainerProfileDialogProps> = ({ trainer, open, onOpenChange }) => {
  ...
  const { data: trainerCourses, isLoading: loadingCourses } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        ...
        .select("*, trainings(name_ar, name_en)")
        .eq("trainer_id", trainer!.id);
      return data || [];
    },
    enabled: !!trainer,
  ...
  // Localized location based on site language
  const displayLocation = (() => {
    const countryEntry = COUNTRIES.find(
      (c) => c.code === trainer.country || c.en === trainer.country || c.ar === trainer.country,
    );
    const cityEntry = countryEntry?.cities.find((c) => c.en === trainer.city || c.ar === trainer.city);
    const country = countryEntry ? (isRTL ? countryEntry.ar : countryEntry.en) : trainer.country;
    const city = cityEntry ? (isRTL ? cityEntry.ar : cityEntry.en) : trainer.city;
    return [city, country].filter(Boolean).join(isRTL ? "، " : ", ");
  })();
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-2 mt-2">
                    <Badge
                      className={
                        trainer.status === "active" ? "bg-emerald-500/15 text-emerald-500 border-emerald-500/30" : ""
                      }
